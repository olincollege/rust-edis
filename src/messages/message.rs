...
impl<T: MessagePayload> Message<T> {
    pub fn serialize(&self) -> Result<Vec<u8>> {
        ...
        let total_length = (size_of_val(&total_length) + size_of_val(&message_type) + message_payload.len()) as u32;
    ...
    pub fn deserialize(buffer: &[u8]) -> Result<Self> {
        let total_length: u32 = u32::from_le_bytes(buffer.get(0..4).context("failed to get total length")?.try_into()?); 
        let message_type_bytes = u8::from_le_bytes(buffer.get(4..5).context("failed to get message type")?.try_into()?);
        let message_type = MessageType::try_from(message_type_bytes).map_err(|_| anyhow::anyhow!("invalid message type"))?;
        ...
        let message_payload = T::deserialize(buffer.get(header_length..header_length + payload_length).context("failed to get message payload")?)?;
        Ok(Message { message_type, message_payload })
    }
}
...
mod tests {
    ...
    fn test_basic_roundtrip() {
        ...
        assert_eq!(message.message_payload.key, deserialized.message_payload.key);
        assert_eq!(message.message_payload.value, deserialized.message_payload.value);
    }
}